...
# HGSOC dataset, no doublets / 40pc debris simulation
...
# barcodes
fn_barcodes_merged <- "../../../supplementary_debris/scenarios/HGSOC/nodoublets/barcodes_merged_HGSOC_nodoublets_debris40pc.tsv"
...
for (i in 1:length(scenario_names)) {
  ...
  # Vireo scenarios
  if (i %in% c(1, 2)) {
    fn <- paste0("../../../supplementary_debris/scenarios/HGSOC/nodoublets/debris40pc/", scenario_names[i], "/vireo/donor_ids.tsv")
    out <- read_tsv(fn)
...
ggplot(df_plot, aes(x = recall, y = precision, color = scenario, shape = sample_id)) + 
  ...
  xlim(0, 1) + 
  ylim(0, 1) + 
  ggtitle("HGSOC, no doublets, 40% debris") + 
  theme_bw()

ggsave("../../../plots/supp_debris/precision_recall_HGSOC_nodoublets_debris40pc.pdf", width = 6.15, height = 3.5)
ggsave("../../../plots/supp_debris/precision_recall_HGSOC_nodoublets_debris40pc.png", width = 6.15, height = 3.5)

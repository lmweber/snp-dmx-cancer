...
# lung dataset, 30pc doublets / 20pc debris simulation
...
# barcodes
fn_barcodes_merged <- "../../../supplementary_debris/scenarios/lung/30pc/barcodes_merged_lung_doublets30pc_debris20pc.tsv"
...
for (i in 1:length(scenario_names)) {
  ...
  # Vireo scenarios
  if (i == 1) {
    fn <- paste0("../../../supplementary_debris/scenarios/lung/30pc/debris20pc/", scenario_names[i], "/vireo/donor_ids.tsv")
    out <- read_tsv(fn)
  ...
  # Vireo scenarios
  if (i == 1) {
    levels(df_truth_tmp$predicted)[1:6] <- c("T28", "T20", "T09", "T08", "T25", "T31")
  }
...
ggplot(df_plot, aes(x = recall, y = precision, color = scenario, shape = sample_id)) + 
  ...
  xlim(0, 1) + 
  ylim(0, 1) + 
  ggtitle("Lung, 30% doublets, 20% debris") + 
  theme_bw()

ggsave("../../../plots/supp_debris/precision_recall_lung_doublets30pc_debris20pc.pdf", width = 6.25, height = 3.5)
ggsave("../../../plots/supp_debris/precision_recall_lung_doublets30pc_debris20pc.png", width = 6.25, height = 3.5)

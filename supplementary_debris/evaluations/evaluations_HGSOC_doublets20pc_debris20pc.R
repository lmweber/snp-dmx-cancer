...
# HGSOC dataset, 20pc doublets / 20pc debris simulation
...
# barcodes
fn_barcodes_merged <- "../../../supplementary_debris/scenarios/HGSOC/20pc/barcodes_merged_HGSOC_doublets20pc_debris20pc.tsv"
...
for (i in 1:length(scenario_names)) {
  ...
  # Vireo scenarios
  if (i %in% c(1, 2)) {
    fn <- paste0("../../../supplementary_debris/scenarios/HGSOC/20pc/debris20pc/", scenario_names[i], "/vireo/donor_ids.tsv")
    out <- read_tsv(fn)
  ...
  # # demuxlet scenarios
  if (i == 3) {
    fn <- paste0("../../../supplementary_debris/scenarios/HGSOC/20pc/debris20pc/", scenario_names[i], "/demuxlet.best")
    out <- read_tsv(fn)
...
ggplot(df_plot, aes(x = recall, y = precision, color = scenario, shape = sample_id)) + 
  ...
  xlim(0, 1) + 
  ylim(0, 1) + 
  ggtitle("HGSOC, 20% doublets, 20% debris") + 
  theme_bw()

ggsave("../../../plots/supp_debris/precision_recall_HGSOC_doublets20pc_debris20pc.pdf", width = 6.15, height = 3.5)
ggsave("../../../plots/supp_debris/precision_recall_HGSOC_doublets20pc_debris20pc.png", width = 6.15, height = 3.5)

...
# barcodes
fn_barcodes_merged <- "../../../supplementary_debris/scenarios/lung/nodoublets/barcodes_merged_lung_nodoublets_debris20pc.tsv"
...
for (i in 1:length(scenario_names)) {
  ...
  # Vireo scenarios
  if (i == 1) {
    fn <- paste0("../../../supplementary_debris/scenarios/lung/nodoublets/debris20pc/", scenario_names[i], "/vireo/donor_ids.tsv")
    out <- read_tsv(fn)
  ...
  # Vireo scenarios
  if (i == 1) {
    levels(df_truth_tmp$predicted)[1:6] <- c("T20", "T28", "T25", "T09", "T31", "T08")
  }
...
ggplot(df_plot, aes(x = recall, y = precision, color = scenario, shape = sample_id)) + 
  ...
  xlim(0, 1) + 
  ylim(0, 1) + 
  ggtitle("Lung, no doublets, 20% debris") + 
  theme_bw()

ggsave("../../../plots/supp_debris/precision_recall_lung_nodoublets_debris20pc.pdf", width = 6.15, height = 3.5)
ggsave("../../../plots/supp_debris/precision_recall_lung_nodoublets_debris20pc.png", width = 6.15, height = 3.5)

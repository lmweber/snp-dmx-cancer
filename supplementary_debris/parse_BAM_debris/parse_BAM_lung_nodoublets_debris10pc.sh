...
awk \
'function assign() { cmd = "shuf -n 1 ../../../supplementary_debris/scenarios/lung/nodoublets/debris_remaining_lung_nodoublets_debris10pc.tsv"; cmd | getline assigned; close(cmd); return assigned } 
NR==1 { next } FNR==NR { a[$1]=$1; next } (i=gensub(/.*CB\:Z\:([A-Za-z]+\-[A-Za-z0-9]+).*/, "\\1", 1, $0)) in a { gsub(i, assign()) }1' \
../../../supplementary_debris/scenarios/lung/nodoublets/debris_lysed_lung_nodoublets_debris10pc.tsv - | \

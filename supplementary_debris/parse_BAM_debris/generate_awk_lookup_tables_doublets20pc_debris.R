...
f_sim_debris <- function(prop_debris, dataset_name, file_barcodes_merged) {
  ...
  fn_out_lysed <- file.path(
    ...
    paste0("debris_lysed_", dataset_name, "_doublets20pc_debris", prop_debris * 100, "pc.tsv")
  )
...
for (prop_debris in prop_debris_sims) {
  for (dataset_name in dataset_name_sims) {
    runtime <- system.time({
      file_barcodes_merged <- file.path("../../../benchmarking/scenarios", dataset_name, "20pc", 
                                        paste0("barcodes_merged_", dataset_name, "_20pc.tsv"))

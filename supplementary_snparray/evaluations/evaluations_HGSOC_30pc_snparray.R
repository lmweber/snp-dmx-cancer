...
# barcodes
fn_barcodes_merged <- "../../../benchmarking/scenarios/HGSOC/30pc/barcodes_merged_HGSOC_30pc.tsv"
...
for (i in 1:length(scenario_names)) {
  ...
  # Vireo scenarios
  if (i %in% c(1:3)) {
    fn <- paste0("../../../supplementary_snparray/scenarios/HGSOC/30pc/", scenario_names[i], "/vireo/donor_ids.tsv")
    out <- read_tsv(fn)
  ...
  # # demuxlet scenarios
  if (i == 4) {
    fn <- paste0("../../../supplementary_snparray/scenarios/HGSOC/30pc/", scenario_names[i], "/demuxlet.best")
    out <- read_tsv(fn)
  ...
  # demuxlet scenarios
  if (i == 4) {
    levels(df_truth_tmp$predicted)[18:20] <- c("X4", "X3", "X2")
  }
...
ggsave("../../../plots/supp_snparray/precision_recall_HGSOC_30pc_snparray.pdf", width = 6.7, height = 3.5)
ggsave("../../../plots/supp_snparray/precision_recall_HGSOC_30pc_snparray.png", width = 6.7, height = 3.5)

...
for (i in 1:length(scenario_names)) {
  ...
  # Vireo scenarios
  if (i %in% c(1:3, 5:6)) {
    fn <- paste0("../../benchmarking/scenarios/HGSOC/20pc/", scenario_names[i], "/vireo/donor_ids.tsv")
    out <- read_tsv(fn)
  ...
  # demuxlet scenarios
  if (i == 4) {
    fn <- paste0("../../benchmarking/scenarios/HGSOC/20pc/", scenario_names[i], "/demuxlet.best")
    out <- read_tsv(fn)
...
ggsave("../../plots/main/precision_recall_HGSOC_20pc.pdf", width = 6.25, height = 3.5)
ggsave("../../plots/main/precision_recall_HGSOC_20pc.png", width = 6.25, height = 3.5)

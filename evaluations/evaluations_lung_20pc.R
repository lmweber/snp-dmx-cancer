...
for (i in 1:length(scenario_names)) {
  ...
  # Vireo scenarios
  if (i == 1) {
    fn <- paste0("../../benchmarking/scenarios/lung/20pc/", scenario_names[i], "/vireo/donor_ids.tsv")
    out <- read_tsv(fn)
  ...
  # Vireo scenarios
  if (i == 1) {
    levels(df_truth_tmp$predicted)[1:6] <- c("T31", "T25", "T20", "T09", "T28", "T08")
  }
...
ggsave("../../plots/main/precision_recall_lung_20pc.pdf", width = 6.15, height = 3.5)
ggsave("../../plots/main/precision_recall_lung_20pc.png", width = 6.15, height = 3.5)
